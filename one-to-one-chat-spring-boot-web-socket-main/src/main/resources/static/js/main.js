...
async function findAndDisplayConnectedUsers() {
    try {
        ...
        if (role === "MANAGER") {
            // Managers see only customers, not themselves or other managers
            users = users.filter(user => user.role === "CUSTOMER" && user.id !== userId);
        } else {
            // Customers see only managers, not themselves
            users = users.filter(user => user.role === "MANAGER" && user.id !== userId);
        }
